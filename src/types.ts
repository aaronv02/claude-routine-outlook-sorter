...
export interface MailSummary {
   ...
   */
  assigned?: { categoryId: string; confidence: number; at: string; generation: number };
}
...
export interface Decision {
  messageId: string;
  /** Ranked best-first. Three are kept; benchmarks say that is where the accuracy is. */
  ranked: Suggestion[];
 ...
 *  - 'full'     : subject, sender, and body preview are offered to Claude.
 *  - 'metadata' : subject and sender only. No body, ever.
 *  - 'rules'    : nothing leaves. Sender rules only; unknown senders wait.
 */
...
export interface Settings {
  ...
  /**
   * Bumped whenever the classifier learns something: a correction, a new
   * sender rule, an edited category description.
   *
   * Stamped onto every message we label. A message parked in Needs Review is
   * only worth reconsidering once this has moved on - otherwise two sweeps in a
   * row pay twice to arrive at the same answer.
   */
  generation: number;
}
...
export interface PersistedState {
  ...
  /**
   * Top-level fields from a state payload written by a NEWER build than this
   * one, carried through untouched so saving here doesn't destroy them.
   *
   * roamingSettings follows the mailbox rather than the client, and Outlook
   * web, desktop, and mobile update on their own schedules - so one mailbox
   * genuinely can have two versions of this add-in reading the same state.
   */
